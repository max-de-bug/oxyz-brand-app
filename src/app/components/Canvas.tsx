...
// import ModularCanvas from "./ModularCanvas";
...
//   // Create a ref to track when the canvas was last updated
...
//     // Only render if sufficient time has passed (throttle updates)
//     if (now - lastCanvasUpdateRef.current > 50) {
...
//     // Skip if the canvas dimensions are already set
//     if (canvas.width === canvasWidth && canvas.height > 0) {
//       return;
//     }
...
//     // Parse the aspect ratio for initial height
//     const [widthRatio, heightRatio] = designStoreAspectRatio
//       .split(":")
//       .map(Number);

//     canvas.height = canvasWidth * (heightRatio / widthRatio);
...
//     lastCanvasUpdateRef.current = Date.now();
//   }, [canvasWidth, designStoreAspectRatio, renderCanvas]);
...
//   // Modify the handleImageResize function to check devMode
//   const handleImageResize = useCallback(
//     (dir: string, amount: number) => {
//       // Only allow image resizing if devMode is enabled
//       if (!devMode) return;
...
//     },
//     [
//       devMode, // Add devMode as a dependency
//       // ... existing dependencies ...
//     ]
//   );

//   // Add keyboard shortcuts for image resizing when in Dev Mode
//   useEffect(() => {
//     const handleKeyDown = (e: KeyboardEvent) => {
...
//       // Skip if Dev Mode is not enabled
//       if (!devMode) {
//         return;
//       }

//       // Only handle image resizing when no text or logo is selected
//       const selectedText =
//         textOverlays.find((t) => t.isSelected) ||
//         (textOverlay.isSelected ? textOverlay : null);
//       const selectedLogo = logos.find((logo) => logo.isSelected);

//       if (selectedText || selectedLogo) {
//         return; // Let other handlers manage selected elements
//       }

//       // Check if we have a main image to resize
//       if (imageUrl && mainImage) {
//         // Handle resizing with + and - keys
//         if (e.key === "=" || e.key === "+") {
//           e.preventDefault();
//           const newSize = maxSize + 5;
//           setMaxSize(newSize);
//           showResizeStatus(`Image Size: ${newSize}`);
//           renderCanvas();
//         } else if (e.key === "-" || e.key === "_") {
//           e.preventDefault();
//           const newSize = Math.max(0, maxSize - 5);
//           setMaxSize(newSize);
//           showResizeStatus(`Image Size: ${newSize}`);
//           renderCanvas();
//         }
...
//   // Update the canvas dimensions when aspect ratio changes
//   useEffect(() => {
//     // Skip if we don't have a canvas yet
//     if (!canvasRef.current) {
//       return;
//     }

//     // Skip if Dev Mode is disabled, but persist the aspect ratio in the store
//     if (!devMode) {
//       return;
//     }
...
//     // Store current canvas width in a ref to avoid circular updates
//     const currentWidth = canvas.width;

//     // Calculate height based on current width and new aspect ratio
...
//       // Show status message with aspect ratio
//       showResizeStatus(`Aspect Ratio: ${widthRatio}:${heightRatio}`);
